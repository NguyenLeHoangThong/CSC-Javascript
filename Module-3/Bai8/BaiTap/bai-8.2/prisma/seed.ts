...
async function main() {
  // Delete children before parents (orders reference products via order_items)
  await prisma.orderItem.deleteMany();
  await prisma.order.deleteMany();
  await prisma.product.deleteMany();
  await prisma.category.deleteMany();

  // Users (upsert so re-seeding doesn't fail on duplicate email)
  const adminHash = await bcrypt.hash('Admin@123456', 12);
  const customerHash = await bcrypt.hash('Customer@123', 12);
  await prisma.user.upsert({
    where: { email: 'admin@shop.com' },
    update: { password: adminHash, role: 'admin' },
    create: { name: 'Admin', email: 'admin@shop.com', password: adminHash, role: 'admin' },
  });
  await prisma.user.upsert({
    where: { email: 'customer@shop.com' },
    update: { password: customerHash, role: 'user' },
    create: { name: 'Customer', email: 'customer@shop.com', password: customerHash, role: 'user' },
  });
  console.log('✅ Users: admin@shop.com / Admin@123456, customer@shop.com / Customer@123');
