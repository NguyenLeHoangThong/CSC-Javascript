...
async function main() {
  ...
  // Reset business data — delete children before parents to respect foreign keys.
  ...
  // ── Sample orders (so the /stats dashboard has data) ──
  const sampleOrders = [
    {
      ...
      lines: [
        { idx: 0, quantity: 1 }, // iPhone 15 Pro
        { idx: 8, quantity: 2 }, // AirPods Pro 2
      ],
    },
    {
      ...
      lines: [{ idx: 3, quantity: 1 }], // MacBook Pro 14"
    },
  ];

  for (const o of sampleOrders) {
    const items = o.lines.map((l) => ({
      productId: createdProducts[l.idx].id,
      quantity: l.quantity,
      price: createdProducts[l.idx].price,
    }));
