...
router.get("/", (req: Request, res: Response) => {
  const { genre, author, fromYear, toYear, sort } = req.query;
  ...
  if (author)
    result = result.filter((b) =>
      b.author.toLowerCase().includes((author as string).toLowerCase())
    );

  if (fromYear) result = result.filter((b) => b.year >= Number(fromYear));

  if (toYear) result = result.filter((b) => b.year <= Number(toYear));
